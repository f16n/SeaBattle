...
USE DATABASE seabattle;

CREATE TABLE server (
    name VARCHAR(16) NOT NULL DEFAULT 'battleship',
    motd VARCHAR(80),
    rules BLOB,
    PRIMARY KEY (name)
);
...
CREATE TABLE user (
        ...
        new_password_hash VARCHAR(50) NOT NULL DEFAULT '',
    PRIMARY KEY (name)
);
INSERT INTO user (name, display_name, notify) VALUES ('server','Server', false);
